...
function filterRecentItemsByIgnoredPlaylistRules(
  recent: SpotifyRecentlyPlayedItem[],
  filterData: Awaited<ReturnType<typeof getIgnoredPlaylistFilterData>> | null,
) {
  ...
  return recent.filter((item) => {
    return !shouldIgnoreRecentPlayByRules({
...
function filterSnapshotRecentHistory(
  ...
  return snapshots.map((snapshot) => ({
    ...snapshot,
    recent: filterRecentItemsByIgnoredPlaylistRules(snapshot.recent, filterData),
  }));
}
...
function collectSnapshotArtistSeeds(snapshots: SpotifyDashboardSnapshot[]) {
  const seeds = new Map<string, SpotifyArtist>();

  snapshots.forEach((snapshot) => {
    const pushArtist = (artist: SpotifyArtist | null | undefined) => {
      ...
      if (!existing) {
        seeds.set(artist.id, {
          ...artist,
          genres: getArtistGenres(artist),
          images: artist.images?.length ? artist.images : undefined,
        });
        return;
      }
      ...
      seeds.set(artist.id, {
        ...existing,
        ...artist,
        genres: [...new Set([...(existing.genres ?? []), ...getArtistGenres(artist)])],
        images: existing.images?.length ? existing.images : artist.images,
        popularity: Math.max(existing.popularity ?? 0, artist.popularity ?? 0),
      });
    };
...
async function getStoredArtistMetadataRecordsByIds(artistIds: string[]) {
  const uniqueArtistIds = [...new Set(artistIds.filter(Boolean))];

  if (!hasMongoConfig() || uniqueArtistIds.length === 0) {
    return [] as StoredArtistMetadata[];
  }
...
export async function getMissingArtistMetadataIdsForUser(spotifyUserId: string) {
  ...
  return artistIds.filter((artistId) => {
...
export async function backfillMissingArtistMetadataForUser(spotifyUserId: string, accessToken?: string) {
  ...
  return missingArtistIds.length;
}
...
async function getHistoricalSnapshots(spotifyUserId: string, range: DashboardRange) {
  ...
  try {
    ...
    return snapshots.length > 0 ? filterSnapshotRecentHistory(snapshots, filterData) : [];
  } catch {
    return [] as SpotifyDashboardSnapshot[];
  }
}
...
async function getLatestSnapshot(spotifyUserId: string) {
  ...
  try {
    ...
    const filterData = await getIgnoredPlaylistFilterData(spotifyUserId).catch(() => null);
    return filterSnapshotRecentHistory([snapshot], filterData)[0] ?? null;
  } catch {
    return null;
  }
}
...
async function fetchSnapshot(accessToken: string, spotifyUserId: string): Promise<SpotifyDashboardSnapshot> {
  ...
  const filterData = await getIgnoredPlaylistFilterData(spotifyUserId).catch(() => null);
  const filteredRecent = filterRecentItemsByIgnoredPlaylistRules(recent.items, filterData);
